...
export class ProductService {
  ...
  getAdminProducts(): Observable<ServiceResponse<Product[]>> {
    return this.http
      ...
      .pipe(
        map((response) => {
          response.data = response.data?.filter((product) => !product.deleted) ?? null;
          return response;
        })
      );
  }
      ...
      .pipe(
        map((response) => {
          response.data = response.data?.filter(
            (product) => product.featured && product.visible && !product.deleted
          ) ?? null;
          return response;
        })
      );
  ...
  getProductsByCategory(categoryUrl: string): Observable<ServiceResponse<Product[]>> {
    return this.http
      .get<ServiceResponse<Product[]>>(`${this.baseUrl}/category/${categoryUrl}`)
      .pipe(
        map((response) => {
          response.data = response.data?.filter(
            (product) =>
              product.category?.url.toLowerCase() === categoryUrl.toLowerCase() &&
              product.visible &&
              !product.deleted
          ) ?? null;
          return response;
        ...
        if (products) {
          for (const product of products) {
            ...
            if (product.description != null) {
              ...
              const words = product.description.split(' ').map((word) =>
                word.replace(new RegExp(punctuation.join('|'), 'g'), '')
              );
  ...
  searchProducts(searchText: string, page: number): Observable<ServiceResponse<ProductSearchResult>> {
    const pageResults = 2;
    return this.http
